...
NUM_ITEMS = 2
ACTION_SPACE_SIZE = MAX_SKILLS_PER_UNIT * (ACTION_TARGET_SLOTS * 2) + NUM_ITEMS * ACTION_TARGET_SLOTS + 1
    ...
    def _obs(self, state: BattleState) -> np.ndarray:
        ...
        for i in range(8):
            if i < len(units):
                ...
                vec.extend([0.0] * (5 + len(token_order)))
