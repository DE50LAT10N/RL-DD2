...
def _defaults() -> GameData:
    ...
    heroes = {
        "man_at_arms": HeroArchetype(id="man_at_arms", name="Man-at-Arms", skills=[strike]),
        "plague_doctor": HeroArchetype(id="plague_doctor", name="Plague Doctor", skills=[strike, heal]),
    }
    monsters = {
        "ghoul": MonsterArchetype(id="ghoul", name="Ghoul", skills=[SkillSpec(id="claw", damage_lo=3, damage_hi=6)]),
        "cultist": MonsterArchetype(id="cultist", name="Cultist", skills=[SkillSpec(id="stab", damage_lo=2, damage_hi=5)]),
    }
    tokens = {k: TokenSpec(id=k) for k in [
        "BLOCK", "BLOCK_PLUS", "DODGE", "DODGE_PLUS", "CRIT", "STRENGTH", "WEAK", "DAZE", "STUN", "COMBO",
        "TAUNT", "RIPOSTE", "SPEED", "VULNERABLE", "BLIND", "BURN", "BLEED", "POISON", "DEATHS_DOOR",
        "UNSTOPPABLE", "IMMOBILIZE",
    ]}
    items = {
        "smoke_bomb": CombatItemSpec(id="smoke_bomb", name="Smoke Bomb", gives_target={"BLIND": 1}),
...
def load(data_root: Path | None = None, *, strict: bool = False) -> GameData:
    ...
    for i, h in enumerate(heroes_raw):
        ...
        data.heroes[hid] = HeroArchetype(id=hid, name=str(h.get("name") or h.get("Name") or hid), skills=skills)
    ...
    for i, m in enumerate(monsters_raw):
        ...
        data.monsters[mid] = MonsterArchetype(id=mid, name=str(m.get("name") or m.get("Name") or mid), skills=skills)
